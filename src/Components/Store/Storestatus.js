...
import {FaChevronLeft, FaChevronRight, FaPen, FaXmark, FaStore, FaMapPin, FaBoxesStacked, FaTags} from 'react-icons/fa6';
...
const Storestatus = () => {
        ...
        const getStoreInfo = async () => {
            ...
            if (store_id) {
                try {
                    ...
                    const storeResponse = await axios.get(`http://my-laravel-app.test/api/show/${store_id}`);
                    ...
                    const userResponse = await axios.get(`http://my-laravel-app.test/api/user/${user.id}`);
    ...
    const handleSaveChanges = async (e) => {
        e.preventDefault();
        try {
            const response = await axios.put(`http://my-laravel-app.test/api/show/${store.id}`, editableStoreInfo);
    ...
    const handleSaveWorkingHours = async (e) => {
        e.preventDefault();
        try {
            const response = await axios.put(`http://my-laravel-app.test/api/working-hours/${editableWorkingHours.id}`, editableWorkingHours);
    ...
    return (
        <div className="flex">
            ...
            <div className="ml-[16.67%] flex-row w-5/6 h-screen p-8 max-[320px]:ml-0 max-[320px]:w-full">
                ...
                {store && (
                    <div className="p-8 mb-5 shadow-lg shadow-gray-500 rounded-md bg-[#f8fafe]">
                        <button
                            className="absolute top-[5%] right-[3%] bg-[#4E82E4] text-white flex items-center px-4 py-1 my-2 rounded-lg shadow-md hover:bg-[#6a9aec] transform hover:scale-105 cursor-pointer"
                            onClick={() => {
                        ...
                        <div className="space-y-1">
                            <p className="flex flex-row justify-start items-center"><FaStore className="mr-2"/><strong className="mr-2">Store Name:</strong> {store.storename}</p>
                            <p className="flex flex-row justify-start items-center"><FaMapPin className="mr-2"/><strong className="mr-2">Store Address:</strong> {store.address}</p>
                            <p className="flex flex-row justify-start items-center"><FaTags className="mr-2"/><strong className="mr-2">Store Category:</strong> {store.category}</p>
                            <p className="flex flex-row justify-start items-center"><FaBoxesStacked className="mr-2"/><strong className="mr-2">Backroom Enabled:</strong> {store.backroom ? 'Yes' : 'No'}</p>
                        </div>
                    </div>
                )}
                ...
                {selectedDate && (
                    <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
                        <div className="bg-white p-6 rounded-lg shadow-lg relative">
                            ...
                            <div className="mb-4">
                                <p>Opening: {selectedDate.hours.opening_time}</p>
                                <p>Closing: {selectedDate.hours.closing_time}</p>
                            </div>
                        </div>
                    </div>
                )}
